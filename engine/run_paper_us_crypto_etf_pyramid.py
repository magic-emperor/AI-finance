"""
run_paper_us_crypto_etf_pyramid.py — forward paper-trading monitor for the
PYRAMID-exit Donchian variant on US/crypto/etf (registry.py hypothesis id=5,
"donchian-pyramid-exit-us-crypto-etf").

A THIRD, separate track alongside run_paper_us_crypto_etf.py (plain baseline
exit) and run_paper_fx_commodity.py. Deliberately not a replacement for the
baseline track -- both stay live, since they are two different, individually
registered hypotheses (same entries, different exit management), not one
hypothesis being "upgraded." Uses simulate_v2(mode="pyramid", config.EXIT_V2)
-- adds units at +1R/+2R with decreasing size, shared trailing stop moved to
breakeven after the first add. max_hold_bars comes from config.EXIT_V2 (60),
not config.DONCHIAN (30), matching what was actually tested.

Robustness context as of 2026-09-16 (see registry.py hypothesis id=5):
run_improve.py picked pyramid as the best of 4 exit variants (baseline/
trail/partial+trail/pyramid) on historical OOS expectancy. PBO (combinatorial
purged cross-validation, engine/backtest/pbo.py) = 0.0635 over 504 resampled
time-block splits -- pyramid wins in 8/10 blocks, a consistent, well-
supported pick, not a multiple-comparisons artifact. Combined (train+OOS)
mean R = +0.707 vs baseline's +0.387 on the SAME entries. This has NOT
separately cleared its own 2x-cost/latency stress or deflated-Sharpe checks
(those were run on the baseline exit specifically) -- PBO answers "was
picking this exit mode overfit", not "does this specific configuration
survive cost/latency shocks". Treat as evidence-based but one layer less
independently stress-tested than the baseline track.

Run once a day:
    python -X utf8 -m engine.run_paper_us_crypto_etf_pyramid            # daily snapshot
    python -X utf8 -m engine.run_paper_us_crypto_etf_pyramid --no-log   # look only
"""
...
STATE_PATH = os.path.join(config.OUTPUT_DIR, "paper_state_us_crypto_etf_pyramid.json")
EQUITY_PATH = os.path.join(config.OUTPUT_DIR, "paper_equity_us_crypto_etf_pyramid.csv")
KILL_FLAG_PATH = os.path.join(config.OUTPUT_DIR, "paper_us_crypto_etf_pyramid_KILLED.json")

# Same pre-committed kill criteria as the other two tracks, same reasoning.
...
def main():
    ...
    for sym in UNIVERSE:
        ...
        if df is None or len(df) < config.DONCHIAN["sma_trend"] + 10:
            continue
        tr = simulate_v2(df, strategy, sym, config.cost_for(sym), "pyramid", config.EXIT_V2)
        per_symbol[sym] = tr
    ...
    print("\n" + "=" * 70)
    print(f"  PAPER MONITOR (US/crypto/etf, PYRAMID exit) — {today} UTC")
