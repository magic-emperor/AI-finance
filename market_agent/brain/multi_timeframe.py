...
BUG FIXES applied in this version (v6 — post-backtest)
-------------------------------------------------------
[BF-13] T1=2.0×ATR produced 29.2% WR (breakeven=27.3%) — technically
        profitable but far below 60% mandate. Root cause: 2.0×ATR T1 on 1H
        is too far. Normal trend consolidation hits 0.75×ATR SL before T1.
        Fix: T1=1.0×ATR, SL=0.75×ATR → 1.33:1 R:R, breakeven=43%.
        Literature (SMA+RSI trending): 55-65% WR → comfortably above target.

[BF-14] No ADX gate in brain. SMA crossovers with ADX<20 are noise — the
        crossover is a random walk, not a trend signal. Added _MIN_ADX=20
        as a per-TF direction gate. A TF with ADX<20 emits HOLD regardless
        of SMA alignment. This is the structural fix for BTC-USD choppy bars.

BUG FIXES applied in this version (v5 — audit)
-----------------------------------------------
...
from market_agent.brain.brain_utils import calc_rsi_series, calc_atr, calc_adx

# ── R:R recalibrated for 60% WR target ───────────────────────────────────────
# [BF-13] T1 was 2.0×ATR (2.67:1 R:R, breakeven 27.3% WR).
# Backtest showed 29.2% WR — technically profitable but far below 60% mandate.
# Root cause: T1=2.0×ATR is too far on 1H. Price hits 0.75×ATR SL during
# normal trend consolidation before ever reaching 2.0×ATR T1.
#
# Fix: T1=1.0×ATR, SL=0.75×ATR → 1.33:1 R:R, breakeven=43% WR.
# Literature (SMA+RSI on trending assets): 55-65% WR achievable → above target.
# T2 kept at 3.5×ATR for partial-exit runners when used in live system.
_RR_T1_MULT = 1.0    # T1 = 1.0 × ATR  →  1.33:1 R:R, breakeven=43%  [BF-13]
_RR_T2_MULT = 3.5    # T2 = 3.5 × ATR (runner target, live system only)
_RR_SL_MULT = 0.75   # SL = 0.75 × ATR (tight, trend invalidation point)

# ── ADX minimum for directional signals ──────────────────────────────────────
# [BF-14] SMA crossovers with ADX < 20 are noise. No directional structure
# means the crossover is a random walk, not a trend entry.
_MIN_ADX = 20.0
...
def multi_timeframe_signal(
    ...
    SELL only if RSI > 40 — don't short into something already oversold.

    ADX gate [BF-14]: ADX < 20 on any TF → HOLD. No directional structure
    means the crossover is noise, not a trend entry.

    R:R [BF-13]: 1.33:1 (T1=1.0×ATR, SL=0.75×ATR). Breakeven=43% WR.
    Literature (SMA+RSI trending): 55-65% WR achievable → above 60% target.
    """
    ...
    for tf, df in timeframes.items():
        ...
        # [BF-14] ADX gate — no directional structure means SMA crossover is noise
        # calc_adx already imported at top level as calc_adx
        adx_local = calc_adx(df.tail(200) if len(df) > 200 else df, 14)

        if sma_valid and sma_slow > 0 and rsi_valid and adx_local >= _MIN_ADX:
            direction = (
        ...
        tf_signals[tf] = {
            ...
            'adx':          float(adx_local),   # [BF-14] stored for measurements
        }
    ...
    # Per-TF ADX values [BF-14]
    for tf, v in tf_signals.items():
        measurements[f'{tf}_adx'] = round(float(v['adx']), 2)
    ...
    primary_adx = float(tf_signals.get('1h', {}).get('adx', -1.0))
    measurements.update({
        ...
        'decision_factor':   1.0 if direction == 'BUY' else (-1.0 if direction == 'SELL' else 0.0),  # A12
        ...
        'primary_adx':       round(primary_adx, 2),   # [BF-14] for Boss Brain
        'adx_gate_min':      float(_MIN_ADX),          # [BF-14] threshold used
    })
    ...
    return BrainSignal(
        ...
        method='SMA (B9 adaptive) + RSI 60/40 + ADX≥20 gate on 1h/4h/1d | B8 weighted votes',
        direction=direction,
        ...
        reliability_flags={
            ...
            'low_adx':            primary_adx < _MIN_ADX and primary_adx >= 0,  # [BF-14]
        },
