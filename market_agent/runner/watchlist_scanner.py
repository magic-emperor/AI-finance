...
EQUITY_SYMBOLS = [s for s in WATCHLIST if ".NS" in s or s in ("NVDA", "GOOGL", "AAPL", "AMD", "MSFT", "META", "AMZN", "TSLA")]
...
def _is_market_open(symbol: str) -> bool:
    ...
    if '.NS' in symbol or 'NIFTY' in symbol.upper():
        ...
        # FX and Commodities: Mon–Fri only (near 24/5)
        return now_utc.weekday() < 5

    else:
        # US Equities (AAPL, AMD, NVDA etc): Mon-Fri 09:30–16:00 ET
        # FIX: Was only checking weekday — no hour check meant brains fired at 2am ET
        # on stale close data. Added proper ET hour gate.
        # Using UTC-4 (EDT) throughout — in winter (EST=UTC-5) this opens 1h early,
        # which is acceptable for paper trade (slightly more conservative).
        ET = _tz(_td(hours=-4))
        now_et = now_utc.astimezone(ET)
        hour_et, minute_et, weekday_et = now_et.hour, now_et.minute, now_et.weekday()
        if weekday_et >= 5:
            return False
        cur_min_et = hour_et * 60 + minute_et
        open_et    = 9 * 60 + 30   # 09:30 ET
        close_et   = 16 * 60 + 0   # 16:00 ET
        return open_et <= cur_min_et <= close_et
