"""
market_agent/runner/run_amv_uptrend_grid.py  (v4.1 — D1 timeframe)

AMV-LSTM-Uptrend Parameter Grid Search — DAILY TIMEFRAME

v4.1 changes (from forward-outcome diagnostic 2026-03-11):
  RSI ceiling: 55 → 60  (D1: RSI 55-60 EV=+0.400R ✅  RSI 60-65 EV=-0.125R ❌)
  close_pct: now range [0.50, 0.70]  (>0.70 EV=-0.102R — exhausted candles)
  SMA slope: 0.10% → 0.05%  (0.10% was filtering without quality benefit)
  All gate constants imported from brain file — single source of truth.
...
WHAT IS SEARCHED:
  ...
  (RSI gates, close_pct range, ATR factor, SMA slope all fixed — evidence-driven)
...
def make_brain_fn(
    ...
    D1-specific notes in this factory:
      ...
      - SMA slope threshold uses _MIN_SMA20_SLOPE_PCT (= 0.05, imported from brain)
    ...
    def _brain(hist: pd.DataFrame, regime: str = 'TRENDING_UP') -> BrainSignal:
        ...
        # ── Gates — mirrors amv_lstm_uptrend.py (D1 v4.1) exactly ──────────────
        # v4.1: _MIN_SMA20_SLOPE_PCT = 0.05 (imported from brain)
        ...
        if close_pct > _MAX_CLOSE_ABOVE_LOW_PCT:          # 0.70 from brain (v4.1 new)
            return _h('Exhausted D1 bounce', factor='EXHAUSTED_BOUNCE_GATE')
        if rsi > 70:
        ...
        if rsi > _RSI_PULLBACK_MAX:                       # 60 from brain (v4.1: 55→60)
        ...
        # v4.2 FIX: threshold changed -0.05% → -0.20% (see brain file Gate 8 notes)
        # v4.2 FIX: NaN guard added to w1_sma10.iloc[-5]
        try:
            ...
            if len(weekly) >= _MIN_W1_BARS:
                ...
                if (w1_sma10.iloc[-5] is not None
                    ...
                    if w1_slope < -0.20:   # -0.20% = genuine declining weekly trend
